...
async def create_user(user_data: Dict) -> User | None:
    ...
    try:
        ...
        if await get_document_by_dict({KeysNames.USERNAME: user_data[KeysNames.USERNAME]}, CollectionNames.USERS.value):
            logger.warning(f"El nombre de usuario {user_data[KeysNames.USERNAME]} ya existe.")
            return None
...
async def authenticate_user(credentials: Dict) -> UserLoginResponse | None:
    ...
    try:
        ...
        user_indb = await get_document_by_dict({KeysNames.USERNAME: credentials[KeysNames.USERNAME]}, CollectionNames.USERS.value)
        ...
        logger.info(f"Usuario autenticado exitosamente: {credentials[KeysNames.USERNAME]}")
